...
describe('server', () => {
  ...
  describe('homepage', () => {
    ...
    it('should contain posts', (done) => {
      superagent.get(`http://localhost:${port}`).end((error, res) => {
  ...
  describe('article page', () => {
    it('should display text or 401', (done) => {
      ...
      seedArticles.forEach((item, index, list) => {
        superagent.get(`http://localhost:${port}/articles/${seedArticles[index].slug}`).end((error, res) => {
          if (item.published) {
            expect(error).to.be(null);
            expect(res.status).to.contain(seedArticles[index].text);
          } else {
